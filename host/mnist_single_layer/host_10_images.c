...
#include "mnist_cnn1_tflite_data_10img.h"
...
static void pack_conv_core(int cid, int filt, int img_idx){
  ...
  for(int y = 0; y < CNN1_IN_H; ++y){
    for(int x = 0; x < CNN1_IN_W; ++x){
      in[y * CNN1_IN_W + x] = cnn1_input_10[img_idx][y][x];
    }
  }
...
static int gather_conv_and_check(int img_idx){
  int errors = 0;
...

    {
      ...
      for(int oy = 0; oy < CNN1_OUT_H; ++oy){
        for(int ox = 0; ox < CNN1_OUT_W; ++ox){
          ...
          if(out[idx_local] != cnn1_conv_out_ref_10[img_idx][c][oy][ox]) errors++;

          /* NHWC flatten order: y, x, channel */
...
static int gather_fc_and_check(int img_idx){
  int errors = 0;
  int Oc[NB_CORES];
  ...
  for(int i = 0; i < 10; ++i){
    if(g_out_hw[i] != cnn1_output_ref_10[img_idx][i]) errors++;
  }
...
int main(void){
  ...
  for(int img_idx = 0; img_idx < CNN1_NUM_TEST_IMAGES; ++img_idx){
    ...
    xil_printf("\r\n--- Image %d ---\r\n", img_idx);
    xil_printf("True label : %d\r\n", (int)cnn1_true_labels_10[img_idx]);
    xil_printf("Ref pred   : %d\r\n", (int)cnn1_ref_preds_10[img_idx]);
    ...
    for(int c = 0; c < NB_CORES; ++c){
      ...
      pack_conv_core(c, c, img_idx);
    }

    run_conv_all();
    conv_errors = gather_conv_and_check(img_idx);
    ...
    run_fc_all();
    fc_errors = gather_fc_and_check(img_idx);
    ...
    xil_printf("REF out : ");
    for(int i = 0; i < 10; ++i) xil_printf("%d ", (int)cnn1_output_ref_10[img_idx][i]);
    xil_printf("\r\n");

    hw_pred = argmax10(g_out_hw);
